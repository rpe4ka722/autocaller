...
def create_abonent(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            phone_list = Abonent.objects.all().values_list('mobile_phone_number', flat=True).exclude(mobile_phone_number=None)
            ...
            work_phone_list = Abonent.objects.all().values_list('work_phone_number', flat=True).exclude(work_phone_number=None)
            if mobile_phone_number in phone_list:
                ...
                if patronymic is None:
                    patronymic =''
                abon = Abonent(first_name=first_name, last_name=cd['last_name'], patronymic=patronymic, 
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                work_phone_list = Abonent.objects.all().exclude(id=id)
                work_phone_list = work_phone_list.values_list('work_phone_number', flat=True).exclude(work_phone_number=None)
                if mobile_phone_number in phone_list and mobile_phone_number is not None:
                    abon = Abonent.objects.get(mobile_phone_number=mobile_phone_number)
                    num = cd['mobile_phone_number']
                    msg = f'Ошибка! Номер { num } указан как мобильный для абонента {abon.full_name()}.'
                elif mobile_phone_number in sec_phone_list and mobile_phone_number is not None:
                    num = cd['mobile_phone_number']
                    abon = Abonent.objects.get(secondary_mobile_phone_number=mobile_phone_number)
                    msg = f'Ошибка! Номер { num } указан как дополнительный для абонента {abon.full_name()}.'
                elif secondary_mobile_phone_number in phone_list and secondary_mobile_phone_number is not None:
                    abon = Abonent.objects.get(mobile_phone_number=secondary_mobile_phone_number)
                    num = cd['secondary_mobile_phone_number']
                    msg = f'Ошибка! Номер { num } указан как мобильный для абонента {abon.full_name()}.'
                elif secondary_mobile_phone_number in sec_phone_list and secondary_mobile_phone_number is not None:
                    abon = Abonent.objects.get(secondary_mobile_phone_number=secondary_mobile_phone_number)
                    num = cd['secondary_mobile_phone_number']
                    msg = f'Ошибка! Номер { num } указан как дополнительный для абонента {abon.full_name()}.'
                    ...
                    object.work_phone_number =work_phone_number
...
def report_export(request, report_id):
    ...
    for call in calls:
        ...
        call_list = [call.abonent.full_name(), call.phone_type, call.abonent_number, conf, call.user_input, call.incorrect_input_count, pass_confirmed, 
                     call.user_pass_input, call.incorrect_pass_input_count, call.start_time.strftime('%X'), call.end_time.strftime('%X'), answ, end_code]
